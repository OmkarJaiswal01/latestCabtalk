...
export async function sosUpdateTaxiDriver(sosId) {
  ...
  try {
    ...
    const journey = await Journey.findOne({ Asset: brokenAsset._id }).session(
      session
    );
    if (journey) {
      ...
      console.warn(
        "[WARN] No active journey found for broken asset, skipping endJourney."
      );
    }
    await session.commitTransaction();
  } catch (err) {
    await session.abortTransaction();
    console.error("[ERROR] Could not end journey:", err);
  } finally {
    session.endSession();
  }
