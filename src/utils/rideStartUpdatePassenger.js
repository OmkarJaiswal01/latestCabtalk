...
export const startRideUpdatePassengerController = async (req, res) => {
  try {
    ...
    const asset = await Asset.findOne({ driver: driver._id }).populate({
      path: "passengers.passengers.passenger", model: "Passenger", select: "Employee_Name Employee_PhoneNumber",
    });
    ...
    if (!shiftGroup || shiftGroup.passengers.length === 0) {
      return res.status(200).json({ success: true, message: `No passengers found for shift "${Journey_shift}".` });
    }
    ...
    return res.status(200).json({
      success: true, message: `Passengers for shift "${Journey_shift}" have been notified.`, passengerCount: shiftGroup.passengers.length,
    });
  } catch (error) {
