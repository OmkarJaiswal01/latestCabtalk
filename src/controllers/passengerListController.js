...
export const sendPassengerList = async (req, res) => {
  try {
    ...
    if (
      ...
      // console.log(
      //   "[sendPassengerList] no shiftBlock or block empty → returning no-passenger message"
      // );
    ...
    if (rows.length === 0) {
      // console.log(
      //   "[sendPassengerList] all passengers boarded → notifying driver & returning"
      // );
      await sendWhatsAppMessage(
        phoneNumber,
    ...
    // console.log(
    //   "[sendPassengerList] watiPayload =",
    //   JSON.stringify(watiPayload, null, 2)
    // );
