...
export const handleWatiWebhook = asyncHandler(async (req, res) => {
  try {
    ...
    const isAssigned = journey.Asset.passengers.some((shiftEntry) =>
      shiftEntry.passengers.some((p) => {
        const id = p.passenger._id ? p.passenger._id : p.passenger;
        return id.toString() === passenger._id.toString();
      })
    );
    if (!isAssigned) {
      await sendWhatsAppMessage(
